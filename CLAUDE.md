...
- **Haptik + ses bağlandı, animasyon henüz değil**: `GameController._apply`, her `GameEvent`'i hem `hapticIntensityFor` (`game_haptics.dart`) hem `soundEffectFor` (`game_audio.dart`) ile eşleyip sırasıyla `HapticsService` (`hapticsServiceProvider`, impl: `VibrationHapticsService`) ve `AudioService`'e (`audioServiceProvider`, impl: `AudioPlayersAudioService`) iletiyor. Animasyon için aynı `events` listesi hâlâ tüketilmiyor — extension point yorumla işaretli. Frame Destroy / line-clear animasyonları bekliyor. Testlerde platform kanalına/asset'e dokunmamak için `FakeHapticsService`/`FakeAudioService` kullanılıyor; hiçbiri gerçek cihazda henüz denenmedi.
