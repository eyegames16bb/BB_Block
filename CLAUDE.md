...
- **Haptik + ses + üç animasyon türü bağlandı**: `GameController._apply`, her `GameEvent`'i hem `hapticIntensityFor` (`game_haptics.dart`) hem `soundEffectFor` (`game_audio.dart`) ile eşleyip sırasıyla `HapticsService` (`hapticsServiceProvider`, impl: `VibrationHapticsService`) ve `AudioService`'e (`audioServiceProvider`, impl: `AudioPlayersAudioService`) iletiyor. Testlerde platform kanalına/asset'e dokunmamak için `FakeHapticsService`/`FakeAudioService` kullanılıyor; hiçbiri gerçek cihazda henüz denenmedi. `BoardGrid` şu üç görsel geçişi doğrudan board diff'inden (`didUpdateWidget`) türetiyor, hiçbir yeni Riverpod state gerekmeden: parça yerleştirme pop-in, sıra temizleme kırılma, çerçeve yıkımı (daha büyük/yavaş/gecikmeli dalga — karar #4). Skor artışı popup'ı ve gerçek parçacık (toz) efekti hâlâ yok.
