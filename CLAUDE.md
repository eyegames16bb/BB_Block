...
- **Haptik bağlandı, ses/animasyon henüz değil**: `GameController._apply`, her `GameEvent`'i `hapticIntensityFor` (`features/game/application/game_haptics.dart`) ile `HapticIntensity`'ye çevirip `HapticsService`'e (Riverpod: `hapticsServiceProvider`, impl: `VibrationHapticsService`) iletiyor. Ses ve animasyon için aynı `events` listesi hâlâ tüketilmiyor — extension point yorumla işaretli. Frame Destroy / line-clear animasyonları bekliyor. Testlerde platform kanalına dokunmamak için `FakeHapticsService` kullanılıyor, gerçek cihazda henüz denenmedi.
