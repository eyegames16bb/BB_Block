...
4. **Çerçeve kaldırma efekti**: GDD'nin "aynı animasyon" ifadesi yerine ayrı bir **Frame Destroy** efekti kullanılıyor — `SoundEffect.frameDestroy` (ses) + `HapticIntensity.heavy` (haptik) + `BoardGrid._frameClearingOverlay` (görsel: sıra temizlemeden daha büyük/yavaş, `Interval` ile sol-üstten dışa doğru gecikmeli dalga). Toz parçacığı (newton_particles) eklenmedi — mevcut efekt scale/opacity tabanlı, gerçek parçacık sistemi ileride bir cila katmanı.
...
- **Haptik + ses + dört animasyon türü bağlandı**: `GameController._apply`, her `GameEvent`'i hem `hapticIntensityFor` (`game_haptics.dart`) hem `soundEffectFor` (`game_audio.dart`) ile eşleyip sırasıyla `HapticsService` (`hapticsServiceProvider`, impl: `VibrationHapticsService`) ve `AudioService`'e (`audioServiceProvider`, impl: `AudioPlayersAudioService`) iletiyor. Testlerde platform kanalına/asset'e dokunmamak için `FakeHapticsService`/`FakeAudioService` kullanılıyor; hiçbiri gerçek cihazda henüz denenmedi. `BoardGrid` üç görsel geçişi doğrudan board diff'inden (`didUpdateWidget`) türetiyor: parça yerleştirme pop-in, sıra temizleme kırılma, çerçeve yıkımı (daha büyük/yavaş/gecikmeli dalga — karar #4). `GameScreen` ayrıca `ref.listen` ile skor artışını yakalayıp bir "+N" popup'ı yükseltip söndürüyor (`_ScorePopup`). Gerçek parçacık (toz) efekti hâlâ yok — mevcut tüm animasyonlar scale/opacity tabanlı.
