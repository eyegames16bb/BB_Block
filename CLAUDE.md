...
- **Haptik + ses bağlandı, Frame Destroy animasyonu henüz değil**: `GameController._apply`, her `GameEvent`'i hem `hapticIntensityFor` (`game_haptics.dart`) hem `soundEffectFor` (`game_audio.dart`) ile eşleyip sırasıyla `HapticsService` (`hapticsServiceProvider`, impl: `VibrationHapticsService`) ve `AudioService`'e (`audioServiceProvider`, impl: `AudioPlayersAudioService`) iletiyor. Testlerde platform kanalına/asset'e dokunmamak için `FakeHapticsService`/`FakeAudioService` kullanılıyor; hiçbiri gerçek cihazda henüz denenmedi. `BoardGrid`'de hem parça yerleştirme ("pop-in") hem sıra temizleme ("kırılma": solma + dışa doğru ölçeklenme) animasyonu var; özel bir **Frame Destroy** efekti (parçalanma + toz + ayrı ses, bkz. karar #4) hâlâ bekliyor — şu an çerçeve kalktığında normal boş hücre görünümüne aniden geçiyor.
- **Sürükle-bırak**: Parçanın (0,0) hücresi işaretçinin altındaki hücreye çapalanıyor (`pointerDragAnchorStrategy`) ve şekil tahtada kalacak şekilde clamp'leniyor. Kullanışlı ama parmağın altında kalıyor; ileride yukarı ofset/geliştirme yapılabilir. Geçersiz bir yere bırakma da artık `onPlace`'e ulaşıyor — engine tek doğruluk kaynağı, widget'taki `_validator` yalnızca sürükleme sırasındaki hayalet önizleme rengi için (önceden bu ayrım yoktu, geçersiz bırakışlar hiçbir geri bildirim tetiklemiyordu — düzeltildi). `BoardGrid`, bir hücre boştan doluya her geçtiğinde `Curves.elasticOut` ile ölçek animasyonu oynatıyor (`_fillGeneration` sayaçlarıyla anahtarlanmış `TweenAnimationBuilder`); temizlenen hücreler de aniden kaybolmak yerine bir an daha ahşap görünümüyle solup büyüyerek çıkıyor (`_clearingCells`/`_clearGeneration`, `onEnd` ile kendini kaldırıyor).
